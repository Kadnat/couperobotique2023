...
# Création d'un objet VideoCapture pour accéder à la caméra
cap = cv2.VideoCapture(0)

# Boucle principale
while True:
    # Lecture d'une image de la caméra
    ret, frame = cap.read()
    ...
    if ret:
        ...
        # Attente d'une touche pour continuer ou quitter
        key = cv2.waitKey(1) & 0xFF
        
        # Si la touche 'q' est pressée, on quitte la boucle
        if key == ord("q"):
            break
...
# Libération des ressources utilisées par la caméra
cap.release()
